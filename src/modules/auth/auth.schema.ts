...
export const registerValuesSchema = z.object({
  ...
  email: stringInput(z.string().trim().transform((email) => email.toLowerCase())),
...
export const registerSchema = registerValuesSchema.extend({
  ...
  password: z.preprocess(
    ...
    z.string().min(1, "Enter your password.").superRefine((password, ctx) => {
      if (!password) {
        return;
      }
  ...
  organizationName: stringInput(z.string().trim().min(1, "Enter your organization name.")),
});
